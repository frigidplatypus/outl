...
macro_rules! theme_commands {
    ($state:ty) => {
        $crate::tauri_commands! {
            ...
            /// Persist the window-decorations preference
            /// (`[display] window_decorations`). Desktop-only in effect —
            /// the phone has no window chrome — but the preference lives
            /// in the shared config, so both clients read and write the
            /// same field.
            fn set_window_decorations(enabled: bool) -> bool
                => plain $crate::commands::theme::set_window_decorations;

            /// Read the current window-decorations preference.
            fn get_window_decorations() -> bool
                => plain $crate::commands::theme::get_window_decorations;
        }
    };
}
