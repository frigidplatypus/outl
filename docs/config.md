...
| Field | Type | Default | Read by | Effect |
|---|---|---|---|---|
| `icons` | `"nerd-font"` \| `"emoji"` | `"nerd-font"` | TUI only | Selects the TUI chrome icon set. `nerd-font` uses compact Font Awesome / Material Design Nerd Font glyphs; `emoji` uses ordinary Unicode emoji and symbols for terminals without Nerd Font support. |
