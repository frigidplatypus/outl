...
| Decide whether a key is outl's own book-keeping (`id` / `from-template` / `collapsed`) and so hidden from the outline + property editors on every client (the TUI's owner; `@outl/shared/markdown::isInternalKey` is the GUI mirror, both pinned by their own test). Feeds `is_suggestable_key`, so a hidden key is never offered as a new property either | `outl_actions::property::is_internal_key` | `crates/outl-actions/src/property.rs` |
...
| Query result hit (id, handle, text, status, page) | `outl_exec::QueryHit` | `crates/outl-exec/src/runtimes/query.rs` |
